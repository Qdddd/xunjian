...
public class TransformerActivity extends AppCompatActivity {
    ...
    //定义选择的容量
    private Spinner cho_capacity;

    //定义点击选中的类型
    private RadioGroup cho_type;

    //定义选择的型号
    private Spinner cho_model;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cho_capacity=(Spinner)findViewById(R.id.cho_capacity);
        cho_type=(RadioGroup)findViewById(R.id.cho_type);
        cho_model=(Spinner)findViewById(R.id.cho_model);
        ...
        weizhi.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mLocationClient.registerLocationListener(new TransformerActivity.MyLocationListener());
                List<String> perminssionList = new ArrayList<>();
                if (ContextCompat.checkSelfPermission(TransformerActivity.this,Manifest.permission.ACCESS_FINE_LOCATION) !=
                        PackageManager.PERMISSION_GRANTED){
                    perminssionList.add(Manifest.permission.ACCESS_FINE_LOCATION);
                }
                if (ContextCompat.checkSelfPermission(TransformerActivity.this , Manifest.permission.READ_PHONE_STATE) !=
                        PackageManager.PERMISSION_GRANTED){
                    perminssionList.add(Manifest.permission.READ_PHONE_STATE);
                }
                if (ContextCompat.checkSelfPermission(TransformerActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) !=
                        PackageManager.PERMISSION_GRANTED){
                ...
                if (!perminssionList.isEmpty()){
                    ...
                    ActivityCompat.requestPermissions(TransformerActivity.this, permissions , 1);
                }else {
                    requestLocation();
                }
            }
        ...
        fl_baocun.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (et_name.getText().toString().equals("") || longitude.getText().toString()==""){
                    AlertDialog alertDialog2 = new AlertDialog.Builder(TransformerActivity.this)
                            .setTitle("提示")
                    ...
                    if (flag == 1) {
                        Toast.makeText(TransformerActivity.this, "已保存", Toast.LENGTH_SHORT).show();
                    } else {
                        ...
                        String bc_cho_capacity = (String) cho_capacity.getSelectedItem();       //获取当前选择的杆塔
                        String bc_run = type_radioButton.getText().toString();           //定义一个变量接收所选类型
                        String bc_cho_model = (String) cho_model.getSelectedItem();       //获取当前选择的杆塔
                        String bc_longitude = longitude.getText().toString();
                        String bc_latitude = latitude.getText().toString();
                        //设置内容不可编辑
                        et_name.setEnabled(false);
                        cho_capacity.setEnabled(false);
                        cho_type.getChildAt(0).setEnabled(false);           //设置type_RadioButton中的第一个按钮为不可编辑状态
                        cho_type.getChildAt(1).setEnabled(false);
                        cho_model.setEnabled(false);
                        Log.d("输入的名字", "" + bc_name);
                        Log.d("选择的容量", "" + bc_cho_capacity);
                        Log.d("选择的类型", "" + bc_run);
                        Log.d("选择的型号", "" + bc_cho_model);
                        Log.d("经度", "" + bc_longitude);
                        Log.d("纬度", "" + bc_latitude);


                        AlertDialog alertDialog1 = new AlertDialog.Builder(TransformerActivity.this)
                                //.setTitle("这是标题")//标题
                                .setMessage("保存成功")//内容
